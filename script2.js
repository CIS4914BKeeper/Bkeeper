document.addEventListener("DOMContentLoaded", function () {
  ...
  submitRequestBtn.addEventListener("click", function (event) {
    ...
    if (fieldsEmpty) {
      var missingFieldsMessage = document.getElementById(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const classButtonsContainer = document.querySelector(".chart-with-slots .row");

  // Function to create a new class button
  function createClassButton(subject, teacher, classId, department, period) {
    const newClassButton = document.createElement("button");
    newClassButton.classList.add("btn", "btn-secondary", "w-100", "mb-2");
    newClassButton.setAttribute("data-subject", subject);
    newClassButton.setAttribute("data-teacher", teacher);
    newClassButton.setAttribute("data-classId", classId);
    newClassButton.setAttribute("data-department", department);
    newClassButton.setAttribute("data-period", period);
    newClassButton.textContent = `Period ${period}: ${subject}`;
  ...
  addClassButton.addEventListener("click", function () {
    ...
    // Check if any of the fields are empty
    if (!subject || !teacher || !classId || !department || !period) {
      alert("Please provide all class details.");
      return;
    }

    // Create a new class button
    const newClassButton = createClassButton(subject, teacher, classId, department, period);
    classButtonsContainer.appendChild(newClassButton);
...
function requestDemo() {
  ...
  const body = encodeURIComponent(
    "Hello, I am [Name], I would like to request a demo of the product. The best way to contact me is [include contact information]"
  );
